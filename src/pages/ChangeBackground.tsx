...
export function ChangeBackground() {
  const [state, setState] = useState<ChangeBackgroundState>({
    image: null,
    replace: "",
    negative: "",
    result: null,
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        formData.append("replace", state.replace)
        if (state.negative) {
          formData.append("negative", state.negative)
        }

        console.log("Sending request with data:", Object.fromEntries(formData))

        const response = await fetch("https://thenewblack.ai/api/1.1/wf/change-background", {
          method: "POST",
          body: formData,
        })
        ...
        if (!response.ok) {
          throw new Error(`Failed to change background: ${responseText}`)
        }
    ...
    [state.image, state.replace, state.negative],
  )
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
        <h1 className="text-4xl font-bold text-gray-900 mb-4">Change Background</h1>
        <p className="text-gray-600">Change the background of your image using AI</p>
      </motion.div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              onChange={handleInputChange}
              placeholder="Describe what you DON'T want in the design (optional)"
          ...
          >
            ...
            <span>{state.loading ? "Changing Background..." : "Change Background"}</span>
          </motion.button>
        ...
        {state.result && (
          ...
          >
            <img
              ...
              alt="Image with changed background"
